import { BXInfo, GDWInfo, WSInfo, WYInfo, YBInfo, TNTSInfo, FlattenInfo } from './GeologyInfo';
...
class GeologyExpose {
  ...
  controlFlatten(active) {
    if (this._flatten === undefined) {
      ...
      this._flatten = new TerrainFlatten({
    ...
    if (active) {
      this._flatten.activate();
    } else {
      this._flatten.recover();
    }
  }
