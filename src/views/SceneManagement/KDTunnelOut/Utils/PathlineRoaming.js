...
function PathinglineRoaming(option) {
  ...
  for (let i = 0; i < this.positions.length - 1; i++) {
    let prePosition = Cesium.Cartesian3.fromDegrees(this.positions[i][0], this.positions[i][1], this.positions[i][2]);
    let currentPosition = Cesium.Cartesian3.fromDegrees(this.positions[i + 1][0], this.positions[i + 1][1], this.positions[i + 1][2]);
...
PathinglineRoaming.prototype.startRoaming = function () {
  ...
  let _this = this;
  //逐点移动
  let flyForwad = function (positions) {
...
PathinglineRoaming.prototype.startRoamingWithoutOri = function () {
  ...
  let flyForwad = function (positions) {
    ...
    //设置相机朝向
    let center = _this.viewer.camera.position;
    let target = Cesium.Cartesian3.fromDegrees(positions[0][0], positions[0][1], positions[0][2]);
    let heading = getHeading(center, target);
    let pitch = getPitch(center, target);
    _this.viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(positions[0][0], positions[0][1], positions[0][2]),
      orientation: {
        heading: heading,
